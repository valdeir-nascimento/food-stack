package io.github.food.stack.domain.order;

import io.github.food.stack.domain.core.Identifier;

import java.util.Objects;
import java.util.UUID;

public class OrderItemID extends Identifier<String> {

    private final String value;

    public OrderItemID(final String value) {
        this.value = Objects.requireNonNull(value);
    }

    public static OrderItemID unique() {
        return OrderItemID.from(UUID.randomUUID());
    }

    public static OrderItemID from(final String id) {
        return new OrderItemID(id);
    }

    public static OrderItemID from(final UUID id) {
        return new OrderItemID(id.toString().toLowerCase());
    }

    @Override
    public String getValue() {
        return value;
    }

    @Override
    public boolean equals(final Object o) {
        if (this == o)
            return true;
        if (o == null || getClass() != o.getClass())
            return false;
        final OrderItemID that = (OrderItemID) o;
        return getValue().equals(that.getValue());
    }

    @Override
    public int hashCode() {
        return Objects.hash(getValue());
    }
}
